...
saved_weight_dir = './check_points/saved_weights_cascade4_' + str(training_number) + '.pth'
tensorboard_file = 'runs/bootstrap_cascade4_' + str(training_number)
...
############################  224 block  ############
        ...
        for index, out in enumerate(outputs.cpu().detach().numpy()):
            w_center, h_center = heatmap_to_coor(out.squeeze())
            cropped_image, cropped_hmap = crop(origin_imgs[index], w_center, h_center, coors[index], 224)
...
############################  56 block  ############
        ...
        if (epoch+1) % 50 == 0:    # every 20 mini-batches...
            ...
            with torch.no_grad():
                ...
                writer.add_scalar("cascade4_Valid_loss", valid_loss, epoch)
                writer.add_scalar("cascade4_Valid_Euclidean_Distance", e_distance/len(valid_loader.dataset), epoch)
