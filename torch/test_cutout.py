...
def eval_cutout():
    ...
    csv = './csv/pass_valid_test_100.csv'
    # csv = './csv/pass_valid_head_6415.csv'
    dir_weight = 'check_points/classifier_cutout_50.pth'
    dataset = CutoutDataset(csv_file=csv)
    ...
    with torch.no_grad():
        for i, batch_data in enumerate(dataloader):
            ...
            image_fake = batch_data['coor_cutout'].cuda()
...
def eval_prediction():
    batch_size = 100
    invalid_batch_size = 5
    # csv = './csv/pred_pass_valid_test_100.csv'
    csv = './csv/pred_pass_valid_head_6415.csv'
    # csv = './csv/1000pred.csv'
    dir_weight = 'check_points/classifier_cutout_50.pth'
    dataset = CutoutDataset_pred(csv_file=csv)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
    classifier = Classifier().cuda()
    classifier.load_state_dict(torch.load(dir_weight))
    classifier.eval()
    ...
    with torch.no_grad():
        ...
        # print('positive / total: {}/{}'.format(
                ...
                # total,
                # ))
    with open('analysis_cutout_50.csv', 'w') as f:
